...
3. 在用魔法编译的时候，在rpc文件中非 API 导出的函数，会出现一些意外的错误
4. 目前整体的错误收集问题也很大，接口的错误抛出逻辑存在问题，另外非手动抛出的异常不得不依赖node全局的错误收集作为fallback手段（这里实际上是将箭头函数直接暴露在模块作用域导致的，这个错误处理问题或许可以有继续使用箭头函数，但输出更优雅的方式来解决）
