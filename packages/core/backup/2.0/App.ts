...
> {
  ...
  async rpcHandle(req: IncomingMessage, res: ServerResponse,next: NextFunction) {
    ...
    // 绑定执行上下文
    const handle = instance[methodKey].bind(instanceProxy);
    // 解析出来payload 交给handle处理 沿用旧的解析逻辑
    const payload = await bodyParse(req);
    if(payload.error){
      // @todo 抛出 400 错误
    }
    // const result = await handle(payload.data).then(res=>res||null);
    await next()
    // @todo 在这里要把数据放到res上
  }
}
